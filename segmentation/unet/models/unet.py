...
class Unet_Encoder(nn.Module):
    def __init__(self, model_name, IN_CHANNEL):
        ...
        if model_name == 'seresnext50':
            ...
            inplanes = 64
            layer0_modules = [
                ('conv1', nn.Conv2d(IN_CHANNEL, 64, 3, stride=2, padding=1,
                                    bias=False)),
...
class Unet_Decoder(nn.Module):
    ...
    def forward(self, x1, x2, x3, x4):
        ...
        x3 = nn.UpsamplingBilinear2d(size=(x2.size(2), x2.size(3)))(x3)
        x2 = self.UP2(torch.cat([x2, x3], 1))
        x2 = nn.UpsamplingBilinear2d(size=(x1.size(2), x1.size(3)))(x2)
...
class Unet(nn.Module):
    def __init__(self, model_name, IN_CHANNEL, NUM_CLASSES, WIDTH, HEIGT):
        ...
        self.model_name = model_name
        self.encoder = Unet_Encoder(model_name, IN_CHANNEL)
        self.decoder = Unet_Decoder(model_name, NUM_CLASSES, WIDTH, HEIGT)
