...
parser.add_argument('--model', type=str, default='efficientnet-b4', required=False, help='specify the backbone model')
...
parser.add_argument("--num_epoch", type=int, default=18, required=False, help="specify the total epoch")
...
def unet_training(model_name,
    ...
    ############################################################################### training parameters
    checkpoint_filename = model_type + '/' + model_name + '/' + model_name + "_" + model_type + "_checkpoint.pth"
    ...
    if lr_scheduler_name == "adamonecycle":
        ...
        scheduler = WarmRestart(optimizer, T_max=STAGE, T_mult=1, eta_min=1e-6)
        lr_scheduler_each_iter = False
    else:
        raise NotImplementedError
    ...
    for epoch in range(1, num_epoch+1):
        ...
        if (not lr_scheduler_each_iter):
            ...
            elif epoch > STAGE and epoch < (STAGE + 2):
